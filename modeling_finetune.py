...
# lightweight version of the pretraining student, just like the finetuning model but without the linear layer
class AlignmentVisionTransformer(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self,
                 img_size=224,
...
@register_model
def alignment_vit_base_patch16_224(pretrained=False, **kwargs):
    model = AlignmentVisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    return model