...
class Efficient_Align(nn.Module):
    ...
    def forward(self, video_embeddings, text_embeddings):
        ...
        logits = (text_embeddings @ video_embeddings.T) * self.logit_scale # torch.ones([]) * 0.07
        targets = torch.nn.functional.softmax(
            texts_similarity, dim=-1
        )
        # targets = texts_similarity
    ...
    def get_video_embeddings(self, video_embeddings):
        bs = video_embeddings.shape[0]
        video_embeddings = self.linear_layer(video_embeddings)
...
class Clip_Frame_Encoder(nn.Module):
    ...
    def forward(self, videos):
        ...
        with torch.no_grad():
            ...
            frame_embeddings = frame_embeddings.view(bs, f // 2, -1, 768)
            # Reshape to [32, 8x196, 768]
            video_embeddings = frame_embeddings.permute(0, 2, 1, 3).contiguous().view(bs, -1, 768)
            print(video_embeddings.shape)
            return video_embeddings
...
def main(args, ds_init):
    ...
    train_video_embeddings.clone().detach().to(device)
